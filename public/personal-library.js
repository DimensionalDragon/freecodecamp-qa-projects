$( document ).ready(function() {
  ...
  $.getJSON('/personal-library/api/books', function(data) {
    //let  items = [];
    itemsRaw = data;
  ...
  $('#display').on('click','li.bookItem',function() {
    ...
    $.getJSON('/personal-library/api/books/'+itemsRaw[this.id]._id, function(data) {
      comments = [];
  ...
  $('#bookDetail').on('click','button.deleteBook',function() {
    $.ajax({
      url: '/personal-library/api/books/'+this.id,
      type: 'delete',
  ...
  $('#newBook').click(function() {
    $.ajax({
      url: '/personal-library/api/books',
      type: 'post',
      dataType: 'json',
